...
export default function PeterLynchPage() {
  ...
  const lynchValue = gfData?.peterLynchFairValue || analysis?.valuation_methods?.peter_lynch?.current_value || 0
  const pegRatio = gfData?.pegRatio || 0
  const margin = lynchValue && currentPrice > 0 
    ? ((lynchValue - currentPrice) / currentPrice) * 100 
    : 0
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter(d => {
      if (!d.price || !d.peterLynchFairValue || d.price <= 0 || d.peterLynchFairValue <= 0) return false
  ...
  const chartData = useMemo(() => {
    ...
    const lynchValues = filteredData.map(d => d.peterLynchFairValue)
    
    const allValues = [...prices, ...lynchValues]
    ...
    const lynchPath = filteredData.map((d, i) => 
      `${i === 0 ? 'M' : 'L'} ${xScale(i)} ${yScale(d.peterLynchFairValue)}`
    ).join(' ')

    // Fill area
    const fillPath = [
      ...filteredData.map((d, i) => `${i === 0 ? 'M' : 'L'} ${xScale(i)} ${yScale(d.price)}`),
      ...filteredData.slice().reverse().map((d, i) => `L ${xScale(filteredData.length - 1 - i)} ${yScale(d.peterLynchFairValue)}`)
    ].join(' ') + ' Z'
    ...
    const latestMargin = ((latest.peterLynchFairValue - latest.price) / latest.price) * 100

    const premiums = filteredData.map(d => ((d.price - d.peterLynchFairValue) / d.peterLynchFairValue) * 100)
    const avgPremium = premiums.reduce((a, b) => a + b, 0) / premiums.length

    // Find best entry point (closest to fair value)
    let bestEntry = filteredData[0]
    let minDiff = Infinity
    filteredData.forEach(d => {
      const diff = Math.abs(d.price - d.peterLynchFairValue)
      if (diff < minDiff) {
        minDiff = diff
        bestEntry = d
      }
  ...
  const getPegRating = () => {
    if (pegRatio <= 0) return { text: 'N/A', color: 'text-[#8b9eb3]' }
    if (pegRatio < 1) return { text: 'Undervalued', color: 'text-[#00C853]' }
    if (pegRatio < 1.5) return { text: 'Fair', color: 'text-[#FFC107]' }
    if (pegRatio < 2) return { text: 'Pricey', color: 'text-[#FF9800]' }
    return { text: 'Expensive', color: 'text-[#FF5252]' }
  }

  const pegRating = getPegRating()
  ...
  return (
    <div className="min-h-screen bg-[#0d1421] pb-24">
        ...
        <div className="px-4 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-[#8b9eb3]">{symbol} - GARP Valuation</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 space-y-6">
        {/* Main Value Card */}
        <div className="bg-gradient-to-br from-[#1E222D] to-[#2a3a4d] rounded-2xl p-5 border border-orange-500/30 shadow-lg">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-[#8b9eb3]">Lynch Fair Value</span>
            <span className={`text-2xl font-bold ${margin >= 0 ? 'text-[#00C853]' : 'text-[#FF5252]'}`}>
              ${lynchValue.toFixed(2)}
            </span>
          </div>
        ...
        <div className="bg-[#1E222D] rounded-xl p-4 border border-[#2a3a4d]">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-white font-medium">PEG Ratio</span>
            </div>
            <div className="text-right">
              <div className={`text-xl font-bold ${pegRating.color}`}>
                {pegRatio > 0 ? pegRatio.toFixed(2) : 'N/A'}
              </div>
              <div className={`text-xs ${pegRating.color}`}>{pegRating.text}</div>
            </div>
          </div>
          <p className="text-xs text-[#5a6b7d] mt-2">
            PEG &lt; 1 = Undervalued • PEG = 1 = Fair • PEG &gt; 2 = Expensive
          </p>
        </div>

        {/* AI Analysis Card */}
        {analysis?.valuation_methods?.peter_lynch && (
          <div className="bg-gradient-to-br from-purple-900/40 to-indigo-900/30 rounded-xl p-4 border border-purple-500/30">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-white font-semibold">🤖 AI Lynch Analysis</h3>
              {analysis.overall_rating && (
                <div className="text-sm font-bold">{analysis.overall_rating}</div>
              )}
            </div>
            
            {analysis.valuation_methods.peter_lynch.recommendation && (
              <p className="text-sm text-white bg-[#0d1421]/50 rounded-lg p-3">
                {analysis.valuation_methods.peter_lynch.recommendation}
              </p>
            )}
          </div>
        ...
        {chartData && (
          <div className="bg-[#1E222D] rounded-xl p-4 border border-[#2a3a4d]">
            <h3 className="text-white font-semibold mb-3">Price vs Lynch Fair Value History</h3>
            ...
            >
              ...
              {/* Lynch Fair Value line (orange) */}
              <path 
                d={chartData.lynchPath} 
                fill="none" 
                stroke="#f97316" 
                strokeWidth="2"
              />
            ...
            <div className="flex items-center justify-center gap-6 mt-3 text-xs">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[#8b9eb3]">Lynch Fair Value</span>
              </div>
            </div>
            ...
            <div className="mt-4 pt-3 border-t border-[#2a3a4d] grid grid-cols-2 gap-4">
              <div className="bg-[#0d1421] rounded-lg p-3">
                ...
                <div className={`text-lg font-bold ${chartData.avgPremium > 50 ? 'text-[#FF5252]' : chartData.avgPremium > 20 ? 'text-[#FF9800]' : 'text-[#FFC107]'}`}>
                  +{chartData.avgPremium.toFixed(0)}%
                </div>
              </div>
              <div className="bg-[#0d1421] rounded-lg p-3">
                <div className="text-xs text-[#5a6b7d] mb-1">Best Entry ({timeRange})</div>
                <div className="text-lg font-bold text-[#00C853]">
                  {chartData.bestEntry.date.slice(0, 7)}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-[#1E222D] rounded-xl p-5 border border-[#2a3a4d]">
          <h2 className="text-lg font-semibold text-white mb-3">What is Peter Lynch Fair Value?</h2>
          <p className="text-[#8b9eb3] text-sm leading-relaxed mb-4">
            <strong className="text-white">Peter Lynch</strong>, legendary Fidelity fund manager, developed GARP (Growth at Reasonable Price) investing. His fair value formula:
          </p>
          
          <div className="bg-[#0d1421] rounded-lg p-4 mb-4 font-mono text-sm text-center">
            <span className="text-orange-400">Fair Value</span> = EPS × (8.5 + 2 × Growth Rate)
          </div>
          
          <p className="text-sm text-[#8b9eb3] mb-3">
            This is based on the <strong className="text-white">PEG Ratio</strong> concept:
          </p>
          
          <div className="bg-[#0d1421] rounded-lg p-4 font-mono text-sm text-center">
            <span className="text-orange-400">PEG</span> = PE Ratio ÷ EPS Growth Rate
          </div>
        </div>

        {/* When to Use */}
        <div className="bg-[#1E222D] rounded-xl p-5 border border-[#2a3a4d]">
          <h2 className="text-lg font-semibold text-white mb-3">When to Use Lynch Fair Value</h2>
          
          <div className="space-y-3">
            <div className="flex items-start gap-3">
              <span className="text-[#00C853] text-lg">✓</span>
              <div>
                <span className="text-white font-medium">Great for:</span>
                <p className="text-sm text-[#8b9eb3]">Growth stocks with predictable earnings • Consumer-facing companies • Finding entry points during corrections</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <span className="text-white font-medium">Less reliable for:</span>
                <p className="text-sm text-[#8b9eb3]">Hypergrowth tech (may need premium adjustment) • Cyclical stocks • Companies with volatile earnings</p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 p-3 bg-orange-500/10 rounded-lg border border-orange-500/20">
            <p className="text-sm text-orange-400">
              <strong>Pro tip:</strong> For dominant tech platforms, consider adding a 20-30% &quot;technology premium&quot; to account for network effects and switching costs.
            </p>
          </div>
        </div>
        ...
        <div className="bg-[#1E222D]/50 rounded-xl p-4 border border-[#2a3a4d]/50">
          <div className="flex items-start gap-3">
            ...
            <div className="text-xs text-[#8b9eb3]">
              <p>
                Peter Lynch managed Magellan Fund to 29% annual returns (1977-1990). His methods favor &quot;boring&quot; growth companies over speculative bets. This metric works best for companies with stable, growing earnings.
              </p>
            </div>
          </div>
        </div>
