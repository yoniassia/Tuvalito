...
export function EtoroAuthButton() {
  ...
  useEffect(() => {
    ...
    const checkAuth = () => {
      ...
      const loggedIn = cookies.some(c => 
        c.startsWith('logged_in=true') || 
        c.startsWith('etoro_sso_user=') ||
        c.startsWith('etoro_user=')
      )
  ...
  const handleLogin = () => {
    setIsLoading(true)
    window.location.href = '/api/auth/etoro/login'
  }
...
export function EtoroAuthButtonMobile() {
  ...
  if (isLoading) {
    return <div className="w-10 h-10 bg-[#161B22] rounded-full animate-pulse" />
  }
